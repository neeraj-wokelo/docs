...
An API request can include either `company` or `primary_company`, but not both. The `company` parameter matches articles that mention the specified company, whereas `primary_company` returns only articles where the specified company is the primary subject of the news article.