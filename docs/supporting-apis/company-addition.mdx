...
2. **Track** the request to completion, either by polling the status endpoint with that `request_id`, or by configuring a webhook to be notified when the status changes.
...
Match the incoming `request_id` against the one returned at submission to correlate the notification with your original request.
