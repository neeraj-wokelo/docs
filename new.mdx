...
The `akta` CLI is a command-line client for the [Akta REST API](https://api.akta.pro/api/v1) — a sibling of the Akta MCP server. Both are thin clients over the same endpoints; the CLI is the presentation layer for humans and shell scripts. It ships as its own self-contained distribution (`akta-cli`) with no MCP-server code.

## Installation

<Tabs>
  <Tab title="pipx">
    Distributed via tagged GitHub Releases. Quickest path (needs read access to the private repo):

    ```bash
    pipx install "git+https://github.com/Wokelo-AI/Akta-CLI@v0.2.1"
    ```

    The `akta` command depends only on `httpx`, `typer`, and `rich`.
  </Tab>
  <Tab title="uv">
    [`uv`](https://docs.astral.sh/uv/) is Astral's faster modern alternative to pipx. It writes a PATH-aware shim on first run, so no extra setup step is needed:

    ```bash
    uv tool install "git+https://github.com/Wokelo-AI/Akta-CLI@v0.2.1"
    ```

    Requires `uv` to be installed first — see [Astral's install guide](https://docs.astral.sh/uv/getting-started/installation/).
  </Tab>
  <Tab title="pip">
    Use `pip` when you're embedding `akta-cli` in a Python project (rather than using `akta` as a standalone tool — see the **pipx** or **uv** tabs for that):
...
v1 authenticates with an Akta API key (`wk_...`), minted at [playground.akta.pro](https://playground.akta.pro) (sign up → API Keys).
