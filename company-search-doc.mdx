...
The Company Search API resolves a company name, website domain, or UUID into a structured company record. Its primary purpose is to look up the `uuid` needed by the [Company Enrichment](/akta-company-enrichment-doc) and [News](/akta-news-api-doc) APIs when you only have a company name to start with.
...
```text
GET https://api.akta.ai/v1/company/search
```
...
curl --location 'https://api.akta.ai/v1/company/search?name=Canva' \
  --header 'x-api-key: <YOUR_API_KEY>'
```

```bash cURL — by website
curl --location 'https://api.akta.ai/v1/company/search?name=canva.com' \
  --header 'x-api-key: <YOUR_API_KEY>'
```
